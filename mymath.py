...
class _Rational:
    ...
    def __float__(self):
        # Avoid NaNs
        if self.d > 1 << 1023:
            self.d = self.trim(1 << 1023)
        return float(self.n) / float(self.d)

    def __pow__(self, exp, z=None):
        if z is not None:
            raise TypeError('Three-argument form of pow() not supported for Rational class')
...
def _float_to_ratio(f, tolerance=1e-10):
    """
    Convert a floating-point number to its rational approximation.
    :param f: The floating-point number to convert.
    :param tolerance: The tolerance for the approximation.
    :return: A tuple (numerator, denominator) representing the rational number.
    """
    if f.is_integer():  # If the float is actually an integer
        return int(f), 1

    # The main part of the algorithm
    # Initialize the fraction as the integer part of the float
    integer_part = int(f)
    f -= integer_part
    numerator1, denominator1 = 1, 0
    numerator2, denominator2 = integer_part, 1

    # Loop until the difference is within the specified tolerance
    while abs(f - (numerator2 / denominator2)) > tolerance:
        f = 1.0 / (f - int(f))
        integer_part = int(f)
        # Update the two fractions
        numerator1, numerator2 = numerator2, numerator1 + integer_part * numerator2
        denominator1, denominator2 = denominator2, denominator1 + integer_part * denominator2
